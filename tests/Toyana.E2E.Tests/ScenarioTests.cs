...
public class ScenarioTests
{
    ...
    public async Task FullWorkflow_VendorLifecycle_And_Booking()
    {
        ...
        var getVendorReq = new RestRequest($"/api/vendor/vendors/{createdVendorId}");
        ...
        for (var i = 0; i < 15; i++) // Poll up to 7.5s
        {
            var searchReq = new RestRequest("/api/catalog/search");
    ...
    private record VendorResponse(Guid Id, string BusinessName, string TaxId);

    private record CatalogSearchResult(List<VendorCatalogItem> Items, int TotalCount);

    private record VendorCatalogItem(Guid Id, string BusinessName);

    private record BookingResponse(Guid Id, string Status, decimal Amount);
}