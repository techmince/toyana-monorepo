...
public static class SearchEndpoints
{
    ...
    )
    {
        ...
        if (!string.IsNullOrEmpty(category)) query = (IMartenQueryable<VendorReadModel>)query.Where(v => v.Category == category);

        if (!string.IsNullOrEmpty(date) && DateOnly.TryParse(date, out var dateOnly))
            // Must NOT contain the date in BlockedDates
            query = (IMartenQueryable<VendorReadModel>)query.Where(v => !v.BlockedDates.Contains(dateOnly));
