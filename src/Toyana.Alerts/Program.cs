...
if (!string.IsNullOrEmpty(botToken))
    builder.Services.AddHttpClient("telegram_bot_client")
           .AddTypedClient<ITelegramBotClient>((httpClient, sp) => { return new TelegramBotClient(botToken, httpClient); });

var app = builder.Build();

app.UseToyanaObservability();

app.MapPost("/webhook/grafana", async
                                (
                                    [FromBody] GrafanaAlertPayload payload, [FromServices] ITelegramBotClient? bot, [FromServices] IConfiguration config,
